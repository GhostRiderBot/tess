...
class LINE:
  ...
  def sendVideoWithURL(self, to_, url):
      path = '%s/pythonLine-%i.mp4' % (tempfile.gettempdir(),random.randint(0,9))
      r = requests.get(url, stream=True)
      if r.status_code == 200:
          with open(path,'wb') as f:
              shutil.copyfileobj(r.raw,f)
  ...
  def changePG(self, to_, id_):
      ...
      r = requests.post(purl, headers = self._headers, data=data)
      if r.status_code != 201:
          r = requests.post(purl, headers = self._headers, data=data)          
          raise Exception('Upload image failure.')         
      try:
  ...
  def changePP(self, to_, id_,from_):
      ...
      try:
          ...
          raise e
  def downloadImage(self, to_, url):
      path = '%s/pythonLine-%i.data' % (tempfile.gettempdir(),random.randint(0,9))
      r = requests.get(url, stream=True, headers = self._headers)
      if r.status_code == 200:
          with open(path,'wb') as f:
